...
def _search_events(question: str) -> list[dict]:
    """
    Two-query strategy: full keyword string + shortened 3-token fallback.
    Results are merged and deduplicated by event ID.
    """
    full_keywords = _extract_keywords(question)
    tokens = full_keywords.split()
    short_keywords = " ".join(tokens[:3]) if len(tokens) > 3 else full_keywords

    seen_ids: set[str] = set()
    merged: list[dict] = []
    for query in dict.fromkeys([full_keywords, short_keywords]):
