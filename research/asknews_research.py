...
Find recent and historically relevant news that would help a forecaster estimate this question. Focus on concrete facts, dated developments, trend evidence, and sources that bear on the resolution criteria.
""".strip()


async def run_asknews_research(
    title: str,
    resolution_criteria: str = "",
    background: str = "",
    fine_print: str = "",
) -> str:
    query = build_asknews_query(title, resolution_criteria, background, fine_print)
    async with _asknews_request_lock:
